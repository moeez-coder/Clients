...
- **Blitz API** (`https://api.blitz-api.ai`, auth via `x-api-key`, key in the `BLITZ_API_KEY` session env var) — live sourcing API, Agency Premium plan. Endpoints: `/v2/search/companies`, `/v2/search/people`, `/v2/search/employee-finder`, `/v2/search/waterfall-icp-keyword` / `waterfall-icp`, `/v2/jobs/search` + `/v2/jobs/company`, `/v2/enrichment/*` (email, phone, company, domain↔linkedin), `/v2/company/tam-by-jobs`. Call it directly over HTTPS — the `Blitz-API` MCP tool in this session only searches Blitz's documentation, it does not proxy live requests. `max_results` caps at **25**. Also supports `naics_code`, `revenue.min/max`, `keywords.include/exclude`, `employee_count`, `type`, `founded_year`. **Accuracy warnings:** `total_results` disagreed with its own history by ~6x on an identical filter, and NAICS tagging is noisy — verify the companies behind any count (see `clients/vntana/icp-universe-map.md` §6).
- **AI Ark API** (`https://api.ai-ark.com/api/developer-portal`, auth via `X-TOKEN`, key in the `AIARK_API_KEY` session env var) — live sourcing API, 400M+ people / 70M+ company profiles, plus lookalike/signal-based company discovery. No MCP tool for this one at all (not even docs search) — call it directly. Rate limit 5 req/s. Key endpoints: `POST /v1/companies` (company search + lookalike search, 0.1 credit/result), `POST /v1/people` (person search across companies, 0.5 credit/result, returns a `trackId` for email lookup), `GET /v1/payments/credits` (free balance check — 15,100 credits as of the last check). Docs: https://docs.ai-ark.com/. **Gotchas:** it returns `403 error code 1010` to the default `Python-urllib` user-agent — send `User-Agent: curl/8.5.0`. Lookalike seeds must be the target's **LinkedIn company URL**; domain seeds silently return unrelated junk. Company names are at `content[].summary.name`, NAICS at `content[].naics`.
- **Clay** (`mcp__Clay__*`) — company/contact find-and-enrich tools plus this workspace's configured custom subroutines.
- **Prospeo API** — **key confirmed present** in a session env as of 2026-09-15 (`PROSPEO_API_KEY`). Not yet exercised; record base URL, auth header and key endpoints here the first time a session uses it.
- **EXA** (`EXA_API_KEY`, `https://api.exa.ai`, auth via `x-api-key`) — **key confirmed present** 2026-09-15. `POST /findSimilar` is a *web-page* similarity engine, not a company-entity engine: seeded with a company URL it returns Wikipedia/Bloomberg/Owler/LeadIQ pages *about* that company rather than peer companies. Use Websets for entity work; prefer AI Ark lookalikes for company peers.
- **DiscoLike** (`DISCOLIKE_API_KEY`) — **key confirmed present** 2026-09-15; this is the original source of the two 99-company VNTANA DiscoLike configs. Not yet called directly from a session — document base URL, auth and endpoints here when first used.
- **ColdIQ** (`COLDIQ_API_KEY`) — **key confirmed present** 2026-09-15, purpose not yet established. Document here once a session works out what it does.
- Sales Navigator remains unwired — run those steps wherever it is connected and bring the resulting export back here.
