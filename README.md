...
3. **Prospeo API** (`https://api.prospeo.io`, auth via `X-KEY`, key in the `PROSPEO_API_KEY` session env var) — confirmed live 2026-09-24 (`GET /account-information` returned a real account summary: PRO plan, remaining credits). Primarily an email finder/verifier; per the LinkedIn-only sourcing protocol below, only reach for its non-email endpoints unless a campaign specifically needs outbound email.
4. **AI Ark API** (`https://api.ai-ark.com/api/developer-portal`, auth via `X-TOKEN`, key in the `AIARK_API_KEY` session env var) — live sourcing API, 400M+ people / 70M+ company profiles, plus lookalike/signal-based company discovery. No MCP tool for this one at all (not even docs search) — call it directly. Rate limit 5 req/s. Key endpoints: `POST /v1/companies` (company search + lookalike search, 0.1 credit/result), `POST /v1/people` (person search across companies, 0.5 credit/result, returns a `trackId` for email lookup — skip the email lookup itself, we only need the LinkedIn identifier), `GET /v1/payments/credits` (free balance check — 15,100 credits as of the last check). Docs: https://docs.ai-ark.com/.
5. Sourcing configs whose brief mentions DiscoLike, EXA, or Sales Navigator name tools not wired into this session; run those steps wherever they are connected and bring the resulting export back here.
